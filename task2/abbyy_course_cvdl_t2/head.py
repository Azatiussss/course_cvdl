...
class CenterNetHead(nn.Module):
    ...
    def __init__(self, k_in_channels=64, c_classes: int = 2):
        ...
        self.class_net = nn.Sequential(
                                ...
                                nn.ReLU(),
                                nn.Conv2d(k_in_channels, c_classes, kernel_size=1),
                                nn.Softmax(dim=1)
                                )
        self.offset_net = nn.Sequential(
                                ...
                                kernel_size=3, padding=1),
                                nn.ReLU(),
                                nn.Conv2d(k_in_channels, 2, kernel_size=1),
                                nn.Softmax(dim=1)
                                )
    ...
    def forward(self, input_t: torch.Tensor):
        class_heatmap = self.class_net(input_t)
        offset_map = self.offset_net(input_t)
        size_map = self.size_net(input_t)
